...
Поведение при попытке сократить уже существующий URL:

- `POST /` и `POST /api/shorten` возвращают **`409 Conflict`**; в теле ответа — уже имеющийся сокращённый URL.
- `POST /api/shorten/batch` возвращает **`409 Conflict`**, при этом для конфликтующих элементов в ответе используются уже существующие короткие ссылки, а недостающие генерируются и сохраняются.
...
- Дубликаты исходных URL: при повторном сокращении сервис возвращает `409 Conflict` и существующую короткую ссылку вместо создания новой записи.

### Хранение данных

- **PostgreSQL** репозиторий: основное хранилище при заданном DSN; таблицы и индексы создаются при старте.
- Уникальный индекс на `original_url` — предотвращает дубликаты
...
- Метод `Clear()` у всех репозиториев: очистка данных.
